...
class DataLogger:
    ...
    def _init_population_summary_log(self):
        header = [
            ...
            "angular_coverage_entropy_P",  # New ACE for P-cells
        ]
        try:
    ...
    def _init_transition_log(self):
        header = [
            ...
            "fmi_before",
            "angular_coverage_entropy_P_before",  # New ACE
    ...
    def log_population_state(
        ...
        *,
        ...
        fmi: float,
        fmi_random: float,
        angular_coverage_entropy_P: float,  # New
    ):
        if self._population_summary_writer:
            try:
                row = [
                    ...
                    (
                        f"{angular_coverage_entropy_P:.4f}"
                        if not np.isnan(angular_coverage_entropy_P)
                        else "NaN"
                    ),  # New
                ]
    ...
    def log_nutrient_transition(
        ...
        *,
        ...
        angular_coverage_entropy_P_before: float = -1.0,  # New
        ...
        if self._transition_log_writer:
            try:
                row = [
                    ...
                    (
                        f"{angular_coverage_entropy_P_before:.4f}"
                        if angular_coverage_entropy_P_before >= 0
                        and not np.isnan(angular_coverage_entropy_P_before)
                        else (
                            "-1.0" if angular_coverage_entropy_P_before < 0 else "NaN"
                        )
                    ),  # New
    ...
    def log_grid_snapshot_data(
        ...
        # ... (same as before, includes new cell attributes is_adapting_to_N2, lag_N2_remaining) ...
    ...
    def close_logs(self):
        # ... (same as before, ensuring both files are closed) ...
